...
import type { Finding, ScanResult } from "@/lib/scanEngine";
...
export default function ResultsView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setTimeout(() => !cancel && (setResult(data), setPhase("done")), 900);
      } catch {
  ...
  const visibleFindings = unlocked ? result.findings : result.findings.slice(0, 3);
  const lockedFindings = unlocked ? [] : result.findings.slice(3);
    ...
    <div className="mx-auto max-w-5xl px-6 py-10 md:py-16">
      ...
      <div className="mt-6 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        ...
        <button
          ...
          className="self-start md:self-end rounded-full border border-white/15 bg-white/5 hover:bg-white/10 text-sm px-4 py-2"
        >
          {t.scan.share}
        </button>
      </div>
      ...
      <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
        <CountPill label={t.scan.counts.critical} value={result.counts.critical} severity="critical" />
        <CountPill label={t.scan.counts.high} value={result.counts.high} severity="high" />
        <CountPill label={t.scan.counts.medium} value={result.counts.medium} severity="medium" />
        <CountPill label={t.scan.counts.low} value={result.counts.low} severity="low" />
      </div>
      ...
      <div className="mt-10">
        <h2 className="text-xl font-bold">
          ...
          <span className="ml-2 text-white/40 text-sm font-normal">
            {unlocked
              ? format(t.scan.sections.totalSuffix, { n: result.findings.length })
              : format(t.scan.sections.moreHidden, { n: result.findings.length - 3 })}
          </span>
        </h2>
      ...
      {/* EMOTIONAL FOOTER */}
      <div className="mt-16 rounded-3xl border border-white/10 bg-white/[0.02] p-8 md:p-10 text-center">
...
function ScanningState({ url }: { url: string }) {
  ...
  useEffect(() => {
    const id = setInterval(() => setI((v) => (v + 1) % steps.length), 450);
...
function ScoreCard({ result, t }: { result: ScanResult; t: Dict }) {
  ...
  const subline = format(t.scan.subline[tone], {
    percentile,
    inverse: 100 - percentile,
  });
  const toneLabel =
    tone === "danger"
      ? t.scan.score.toneDanger
      : tone === "warn"
      ? t.scan.score.toneWarn
        ...
        <div className="relative h-48 w-48 shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <div className="text-xs text-white/50 mt-1">
              {format(t.scan.score.outOf, { grade })}
            </div>
          </div>
        </div>

        <div className="text-center md:text-left flex-1">
          <div className="text-xs tracking-widest uppercase" style={{ color: ringColor }}>
            {toneLabel}
          </div>
          ...
          <div className="mt-5">
            ...
            <div className="relative h-2 rounded-full bg-white/5 overflow-hidden">
              ...
              />
              <div
                className="absolute inset-y-0 left-0 bg-gradient-to-r from-safe via-warn to-danger opacity-40"
                style={{ width: "100%" }}
              />
            </div>
            <div className="mt-2 text-xs text-white/50">
              {format(t.scan.score.benchmarkText, { percentile })}
            </div>
          </div>
        </div>
      </div>
...
function CountPill({
  ...
  label: string; value: number; severity: Finding["severity"];
}) {
...
function FindingCard({ f, unlocked, t }: { f: Finding; unlocked: boolean; t: Dict }) {
  const s = SEVERITY_STYLES[f.severity];
  const meta = t.scan.findings[f.key];
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-[10px] text-white/40">{meta.category}</span>
          </div>
          <h3 className="mt-2 text-lg font-bold">{meta.title}</h3>
          <p className="mt-1 text-sm text-white/60">{meta.summary}</p>
        </div>
      </div>
